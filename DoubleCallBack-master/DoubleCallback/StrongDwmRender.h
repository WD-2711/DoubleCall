...
{
	PVOID modbase;
	// modbase = dxgi.dll 的基址
	auto pProcess = GetProcessWModule("dwm.exe", "dxgi.dll", &modbase);

	//resolve swapchain
	// ((DWORD64)FindPatternSect(modbase, ".text", "48 8B D3 E8 ? ? ? ? 48 83 A7") + 16 + 7 + *(LONG*)((DWORD64)FindPatternSect(modbase, ".text", "48 8B D3 E8 ? ? ? ? 48 83 A7") + 16 + (7 - sizeof(LONG))))
	// 找到 dxgi.dll 中 IDXGISwapChainDWM 的特征码
	IDXGISwapChainDWM = (PVOID*)EPtr(RVA(FindPatternSect(modbase, ".text", "48 8B D3 E8 ? ? ? ? 48 83 A7") + 16, 7));

	// 找那个地址保存了 IDXGISwapChainDWM 接口初始化的地址，放到 classTest 中
...
int GetDwmPresentThreadId()
{
	...
	// 遍历 dwm.exe 的线程，并根据创建时间拿到当前运行的线程
	int tdid = 0;
	__int64 times = 0;
