...
// 找 PXE、PPE、PDE、PTE 基地址
void InitializePteBase(u64 dirbase) {
	PHYSICAL_ADDRESS phAddr; 
	u64 slot = 0;

	// PML4T 物理基地址的 12-35 位
	auto pfn = PTE{ dirbase }.PageFrameNumber;
	
	// PML4T 物理基地址
	phAddr.QuadPart = pfn << PAGE_SHIFT;

	// 调用 MmGetVirtualForPhysical 将 PML4T 物理基地址转为虚拟地址，并将结果强转为 PTE 类型的指针
	// auto pml4 = (PTE*)((MmGetVirtualForPhysicalDef)EPtr(MmGetVirtualForPhysicalFn))(phAddr)
	auto pml4 = (PTE*)ImpCall(MmGetVirtualForPhysical, phAddr);
	
	// 在 PML4T 的虚拟地址中向下一个一个寻找，每一个结构都是一个 PML4E
	// 找到符合当前 CR3（当前进程）的 PML4E，记录索引（slot）
